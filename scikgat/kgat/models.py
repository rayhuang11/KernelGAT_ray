...
class inference_model(nn.Module):
    def __init__(self, bert_model, args):
        ...
        self.mu = Variable(torch.FloatTensor(kernal_mus(self.kernel)), requires_grad = False).view(1, 1, 1, 21).cpu()  # changed from cuda
        self.sigma = Variable(torch.FloatTensor(kernel_sigmas(self.kernel)), requires_grad = False).view(1, 1, 1, 21).cpu()  # changed from cuda


    def self_attention(self, inputs, inputs_hiddens, mask, mask_evidence, index):
        idx = torch.LongTensor([index]).cpu() # change from cuda
    ...
    def get_intersect_matrix(self, q_embed, d_embed, attn_q, attn_d):
        ...
        pooling_value = torch.exp((- ((sim - self.mu.cpu()) ** 2) / (self.sigma.cpu() ** 2) / 2)) * attn_d

...
class inference_model(nn.Module):
    def __init__(self, bert_model, args):
        ...
        self.mu = Variable(torch.FloatTensor(kernal_mus(self.kernel)), requires_grad = False).view(1, 1, 1, 21) # .cpu()
        self.sigma = Variable(torch.FloatTensor(kernel_sigmas(self.kernel)), requires_grad = False).view(1, 1, 1, 21) # .cpu()
